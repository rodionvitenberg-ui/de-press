...
Принципы: минимальные диффы; parity = перенос существующего, не новые фичи; каждый шаг заканчивается зелёными проверками и коммитом; пока вопросы не подтверждены — идём по рекомендациям (помечены в OPEN_QUESTIONS).
...
| P1.3 | **Calls в Mini App**: по решению Q2 — рекомендация отложить | ожидает Q2 |
...
## P2. Подготовка репозитория

- Лицензия + публичность — после решения Q5.
- `git log --format=%ae | sort -u` — проверить email в истории.
- CI (GitHub Actions): backend (ruff + pytest чанками), browser (vitest + tsc + build), mini-app (build) — при публикации.
- `.env.example` сверить с `settings/base.py`; README: быстрый старт, стек, структура репо.
...
- Формат: EN заменяет RU (рекомендация Q6-A).
...
- Состав v1: живая очередь запросов (поллинг, Q4), «взять» → открытие диалога (кнопка звонка уже в диалоге), статус дежурства (существующий toggle), сводка (существующая), метрики из Q8.
